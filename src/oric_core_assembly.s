...
; ------------------------------------------------------------------------------------------
_OverlayMemCopy_core:
; Function to copy memory to another place in memory with upper 16KiB overlay RAM area enabled
; Input:	ORIC_addrh = high byte of source address
;			ORIC_addrl = low byte of source address
;			ORIC_desth = high byte of VDC destination address
;			ORIC_destl = low byte of VDC destination address
;			ORIC_tmp1 = number of 256 byte pages to copy
;			ORIC_tmp2 = length in last page to copy
; ------------------------------------------------------------------------------------------
...
copyloopbmc:							; Start of copy loop
	...
	sta (ZP3),y 						; Store data at destination
...
nextbmc1:								; Next label
	inc ZP3								; Increment low byte of source address
	bne nextbmc2						; If not yet zero, branch to next label
	inc ZP4								; Increment high byte of source address
nextbmc2:								; Next label

	; Decrease counters
	dec _ORIC_tmp2						; Decrease low byte of length
	lda _ORIC_tmp2						; Load low byte of length to A
	cmp #$ff							; Check if below zero
	bne copyloopbmc						; Continue loop if not yet below zero
	dec _ORIC_tmp1						; Decrease high byte of length
	lda _ORIC_tmp1						; Load high byte of length to A
	cmp #$ff							; Check if below zero
	bne copyloopbmc						; Continue loop if not yet below zero
...
; ------------------------------------------------------------------------------------------
_OverlayMemSet_core:
; Function to set memory to specific value with upper 16KiB overlay RAM area enabled
; Input:	ORIC_addrh = high byte of source address
;			ORIC_addrl = low byte of source address
;			ORIC_value = value to set at all memory positions
;			ORIC_tmp1 = number of 256 byte pages to copy
;			ORIC_tmp2 = length in last page to copy
; ------------------------------------------------------------------------------------------
...
copyloopbms:							; Start of copy loop
	...
	lda _ORIC_value						; Load value to set in A
	sta (ZP1),y							; Load source data
